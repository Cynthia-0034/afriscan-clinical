...
const CollaborationPanel = ({ comments: initialComments }: CollaborationPanelProps) => {
      ...
      <div className="px-5 py-4 space-y-4">
        {comments.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-[11px] text-muted-foreground/70 mt-0.5">Be the first to add a clinical note.</p>
          </div>
        ) : (
          <div className="space-y-2.5 max-h-60 overflow-y-auto">
            {comments.map((c) => (
              <div key={c.id} className={`rounded-lg p-3 ${c.isSecondOpinion ? "bg-accent/80 border border-primary/15" : "bg-secondary/80"}`}>
                <div className="flex items-center gap-2 mb-1.5">
                  <UserCircle className="w-3.5 h-3.5 text-muted-foreground" />
                  <span className="text-[11px] font-bold text-foreground">{c.doctorName}</span>
                  {c.isSecondOpinion && (
                    <span className="text-[9px] font-bold text-primary bg-primary/10 px-1.5 py-px rounded">2nd Opinion</span>
                  )}
        ...
        <div className="border-t pt-3.5 space-y-2.5">
          ...
          <div className="flex gap-2">
            <Button size="sm" onClick={addComment} disabled={!newComment.trim() || !doctorName.trim()} className="h-7 text-xs px-3">
              <Send className="w-3 h-3 mr-1.5" /> Comment
            </Button>
            {!secondOpinionRequested && (
              <Button variant="outline" size="sm" onClick={() => setSecondOpinionRequested(true)} className="h-7 text-xs px-3">
